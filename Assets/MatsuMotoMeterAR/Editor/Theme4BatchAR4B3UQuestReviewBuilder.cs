...
{
    internal static class Theme4BatchAR4B3UQuestReviewBuilder
    {
        internal const string OutputPath =
            "Builds/QuestReview/AnalogInstrumentMR-Theme4-P6-BatchA-R4-B3U-R2-1024-review-quest3.apk";

        private const string CandidateId = "Theme4_P6_BatchA_R4_B3U_R2_1024";
        ...
        private const string QuarantinedDevAgentSettingsPath =
            ...
            "DevAgentSettings.theme4-p6-batch-a-r4-b3u-review.local.asset";
        ...
        private const string CandidateResourcePrefix =
            "Theme4_P6_BatchA_R4_B3U_R2_1024/MachinedErgonomics/Prefabs/";
        private const string ReviewTheme = "OrbitalAnalog";

        private static readonly string[] Models = { "Throttle", "PowerSlider" };

        [MenuItem(
            "Tools/MatsuMotoMeterAR/Theme 4/" +
            "Build Batch A R4 B3U 1024 Quest Review APK")]
        public static void Build()
        {
            ...
            Theme4BatchAR4B3UMaterialGateBuilder.PrepareR2QuestReviewResources1024();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException(
                        $"B3U review build failed: {report.summary.result}; " +
                        $"{report.summary.totalErrors} error(s).");
                Debug.Log($"B3U 1024 Quest review APK built: {OutputPath}; " +
            ...
            finally
            {
                ...
                try
                {
                    if (quarantined) RestoreDevAgentSettings();
                }
                finally
                {
                    if (configurationCreated) DeleteReviewConfiguration();
                }
            }
        }
        ...
        private static ReviewMapping[] ReviewMappings()
        {
            return Models.Select(model => new ReviewMapping
            {
                ...
                resourcePath =
                    $"{CandidateResourcePrefix}PF_Visual_{model}_MachinedErgonomics"
            }).ToArray();
        }
        ...
        private static void CreateReviewConfiguration()
        {
            if (File.Exists(ReviewConfigurationPath))
                throw new IOException("Candidate review configuration exists: " +
            ...
            File.WriteAllText(ReviewConfigurationPath,
                JsonUtility.ToJson(configuration, true), new UTF8Encoding(false));
        ...
        private static void DeleteReviewConfiguration()
        {
            if (!AssetDatabase.DeleteAsset(ReviewConfigurationPath) &&
                ...
                throw new IOException("Could not remove review configuration: " +
                                      ReviewConfigurationPath);
            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
        }
        ...
        {
            if (!File.Exists(DevAgentSettingsPath)) return false;
            if (File.Exists(QuarantinedDevAgentSettingsPath))
                throw new IOException("Credential quarantine exists: " +
            ...
            if (!string.IsNullOrEmpty(error))
                throw new IOException("Could not quarantine credentials: " + error);
            return true;
        }
        ...
        {
            if (!File.Exists(QuarantinedDevAgentSettingsPath))
                throw new FileNotFoundException("Quarantined credentials lost",
                    QuarantinedDevAgentSettingsPath);
            if (File.Exists(DevAgentSettingsPath))
                throw new IOException("Credential restore destination exists: " +
                                      DevAgentSettingsPath);
            var error = AssetDatabase.MoveAsset(
                QuarantinedDevAgentSettingsPath, DevAgentSettingsPath);
            if (!string.IsNullOrEmpty(error))
                throw new IOException("Could not restore credentials: " + error);
        }
