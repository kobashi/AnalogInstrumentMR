...
{
    internal static class Theme4Phase3CandidateBuilder
    {
        private const string SourceRoot =
            "ArtSource/Blender/BrushUp/Opus5/MachinedErgonomics/delivery_p3";
        private const string CandidateRoot =
            "Assets/MatsuMotoMeterAR/Content/RefinedCandidates/" +
            "CandidateStaging/Theme4_P3_Delivery";
        ...
        private const string QuestResourceRoot =
            CandidateRoot + "/Resources/Theme4_P3_Delivery/" +
            "MachinedErgonomics/Prefabs";
        private const string ReportPath =
            "Builds/Reports/candidate-Theme4_P3-delivery-validation.md";
        private const string VisualReviewRoot =
            "Builds/Reports/Theme4_P3_VisualReview";
        ...
        private const string OpaqueMaterialPath =
            MaterialRoot + "/MAT_MachinedErgonomics_P3_Atlas_Staging.mat";
        private const string EmissiveMaterialPath =
            MaterialRoot + "/MAT_MachinedErgonomics_P3_Emissive_Staging.mat";

        private static readonly AssetSpec[] Assets =
        {
            new(
                "MeterRound",
                "SM_MeterRound_MachinedErgonomics_V6_Opus5_P3.fbx",
                "needle_pivot",
                "needle",
                Vector3.forward,
                ...
                new Vector3(0.149f, 0.149f, 0.0426f),
                0.023687f,
                "fe40ccd2375af2126e7e85ad8ea071bcd08fdf7144d4883721bc2547640171f2"),
            new(
                "Lever",
                "SM_Lever_MachinedErgonomics_V6_Opus5_P3.fbx",
                "handle_pivot",
                "handle",
                Vector3.right,
                ...
                0f,
                7852,
                new Vector3(0.238810f, 0.438810f, 0.139710f),
                new Vector3(0f, 0f, 0.140335f),
                new Vector3(0.238810f, 0.438810f, 0.280669f),
                0.0148f,
                "fdb7b210f6ccc86d78ca8aa74c58371ee19e5ecdbaf26e5cf9a9fee73ee8d34a"),
            new(
                "Toggle",
                "SM_Toggle_MachinedErgonomics_V6_Opus5_P3.fbx",
                "switch_pivot",
                "switch",
                Vector3.right,
                ...
                0.032f,
                "a2269eed9f3a04d0ad55ec7eb26720e126c2940f949751f216f626150715ce32")
        };

        [MenuItem(
            "Tools/MatsuMotoMeterAR/Theme 4/" +
            "Build and Validate Phase 3 Candidate")]
        public static void BuildAndValidate()
        {
            ...
            CopyTexture(NormalName,
                "2a37097d002c7245b2a672ff0336249e5af2653458bc882bd86d30d9558ab784");
        ...
        internal static void PrepareQuestReviewResources()
        {
            ...
            foreach (var spec in Assets)
            {
                var source =
                    ...
                    "MachinedErgonomics_P3_Staging.prefab";
                var destination =
        ...
        {
            if (!File.Exists(path))
                throw new FileNotFoundException("Theme 4 P3 input is missing.", path);
            var actual = Sha256(path);
            if (!string.Equals(actual, expected, StringComparison.Ordinal))
            {
                throw new InvalidDataException(
                    $"Theme 4 P3 SHA mismatch for {path}: {actual}");
            }
        }
            ...
            Material emissive)
        {
            ...
            if (source == null)
                throw new FileNotFoundException("Imported P3 FBX is missing.", modelPath);
            ...
            var prefabName = $"PF_Visual_{spec.Key}_MachinedErgonomics_P3_Staging";
        ...
        private static void Validate(Material opaque, Material emissive)
        {
            ...
            report.AppendLine("# Theme 4 Phase 3 isolated candidate validation");
            report.AppendLine();
            report.AppendLine(
            ...
            if (failures.Count > 0)
                throw new InvalidOperationException("Theme 4 P3 candidate validation failed.");
            Debug.Log("Theme 4 P3 isolated candidate PASS: " + ReportPath);
        }
        ...
        {
            var prefabPath =
                $"{PrefabRoot}/PF_Visual_{spec.Key}_MachinedErgonomics_P3_Staging.prefab";
        ...
        private static void RenderVisualReview()
        {
            ...
            WritePng(
                $"{VisualReviewRoot}/ContactSheet_Theme4_P3_Unity_colour.png",
                colourSheet,
                cellSize * 4,
                cellSize * 3);
            WritePng(
                $"{VisualReviewRoot}/ContactSheet_Theme4_P3_Unity_grayscale.png",
                grayscaleSheet,
                cellSize * 4,
                cellSize * 3);
            File.AppendAllText(
                ...
                "- 12 fixed-camera renders: `Builds/Reports/Theme4_P3_VisualReview/Unity_*.png`\n" +
                "- Colour contact sheet: `ContactSheet_Theme4_P3_Unity_colour.png`\n" +
                "- Grayscale contact sheet: `ContactSheet_Theme4_P3_Unity_grayscale.png`\n" +
            ...
            Debug.Log("Theme 4 P3 Unity visual review written to " + VisualReviewRoot);
        }
        ...
        private static RoleLuminance ValidateAtlasRoleLuminance()
        {
            ...
            try
            {
                ...
                {
                    throw new InvalidDataException("Could not decode Theme 4 P3 atlas PNGs.");
                }
                ...
                if (!(gasket < metal && metal < body && body < readout))
                    throw new InvalidDataException("Theme 4 P3 BaseColor role luminance order failed.");
                if (emissionBody > 0.01f || emissionMetal > 0.01f ||
                    emissionGasket > 0.01f || emissionReadout < 0.5f)
                {
                    throw new InvalidDataException("Theme 4 P3 emission quadrant contract failed.");
                }
            ...
            int size)
        {
            ...
            var root = new GameObject("Theme4 P3 Visual Review Root");
            var instance = UnityEngine.Object.Instantiate(prefab, root.transform, false);
            var cameraObject = new GameObject("Theme4 P3 Review Camera");
            var lightObject = new GameObject("Theme4 P3 Review Key Light");
            var fillObject = new GameObject("Theme4 P3 Review Fill Light");
