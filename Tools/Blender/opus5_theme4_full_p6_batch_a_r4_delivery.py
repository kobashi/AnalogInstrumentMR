...
Usage::
    ...
    scripts/run-blender.sh --background --factory-startup \
      --python Tools/Blender/opus5_theme4_full_p6_batch_a_r4_delivery.py -- \
      --project-root "$PWD"
"""
...
OUTPUT = f"{TREE}/theme4_full_p6_batch_a_r4_delivery.json"
...
def main():
    ...
    payload = {
        "phase": "Theme4-P6-BatchA-R4-delivery",
        "note": ("Throttle and PowerSlider with the bore widened until "
                 "a ray cast clears the nominal tool, through the frozen "
                 "P4 atlas. MeterMedium, MeterLarge, Batch B, batch_a_r2, "
                 "batch_a_r3 and P1-P5 are untouched."),
        ...
        "budgets": {"triangle_budget_validator_total": r4.VALIDATOR_TOTAL,
                    "triangle_target_total": r4.TARGET_TOTAL},
        "assets": {},
    }
    ...
    for asset, builder in r4.BUILDERS_R4.items():
        ...
        measured["triangle_budget_total"] = r4.VALIDATOR_TOTAL
        measured["triangle_target_total"] = r4.TARGET_TOTAL
        measured["gates"]["triangles_total"] = (
            measured["triangles_total"] <= r4.VALIDATOR_TOTAL)
        measured["gates"]["triangles_target"] = (
            measured["triangles_total"] <= r4.TARGET_TOTAL)
        ...
        access = r4.tool_path_audit(body, screws, face, base,
                                    tool_r=r4.TOOL_NOMINAL_R)
        fbx = tree / f"SM_{asset}_{THEME}_V6_Opus5_P6A_R4.fbx"
        ...
        for label, view in p1.VIEWS.items():
            path = review_dir / f"Delivery_{asset}_{THEME}_P6A_R4_{label}.png"
        ...
        details.update(r4.tool_access_images(asset, screws, face, base,
                                             detail_dir, project_root))

        submeshes = sum(r["submeshes"] for r in rows.values())
        row = dict(measured)
        row.update({
            "p6a_r4_fbx": str(fbx.relative_to(project_root)),
            "p6a_r4_fbx_sha256": m1.digest(fbx),
            "p6a_r4_fbx_bytes": fbx.stat().st_size,
            "objects": rows,
        ...
        print(f"[BatchA-R4-Delivery] {asset}: tris {row['triangles_total']}, "
    ...
    grey = tree / f"ContactSheet_Theme4_{THEME}_P6A_R4_grayscale.png"
    p1.comparison_sheet(rows, grey)
    colour = tree / f"ContactSheet_Theme4_{THEME}_P6A_R4_colour.png"
