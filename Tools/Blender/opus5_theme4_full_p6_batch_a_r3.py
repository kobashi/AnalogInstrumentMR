"""Batch A R3: the four screw bores cut with a boolean, taper kept.
...
Usage::
    ...
    scripts/run-blender.sh --background --factory-startup \
      --python Tools/Blender/opus5_theme4_full_p6_batch_a_r3.py -- \
      --project-root "$PWD"
"""
...
OUTPUT = f"{TREE}/geometry/theme4_full_p6_batch_a_r3.json"
NEUTRAL = ba.NEUTRAL
TARGET_TOTAL = 4800
...
tool_path_audit = r2.tool_path_audit

BORE_SEG = 12
BORE_CLEAR_R = 0.0082        # the clear radius the tool actually gets
# A 12-sided cutter cuts a 12-sided hole, and its inradius is what a
# driver has to fit through, not its circumradius. The first build
# specified 16.4 mm and measured 15.84, which is exactly cos(15 deg).
...
def part_breakdown(builder):
    ...
    try:
        material = p1.proto.make_material(f"MAT_{THEME}_R3_Neutral", NEUTRAL)
        builder(material)
    finally:
        p1.join = original
...
def main():
    ...
    payload = {
        "phase": "Theme4-P6-BatchA-R3-geometry",
        ...
        "tool": {"bore_diameter_mm": round(BORE_R * 2000.0, 2),
    ...
    import opus5_brushup_kinetic_review as review
    for asset, builder in BUILDERS_R3.items():
        ...
        blend = geometry_dir / f"BL_{asset}_{THEME}_V6_Opus5_P6A_R3.blend"
        ...
        for label, view in p1.VIEWS.items():
            path = grey_dir / f"Grey_{asset}_{THEME}_P6A_R3_{label}.png"
        ...
        print(f"[BatchA-R3] {asset}: tris {row['triangles_total']} "
    ...
    payload["status"] = (
        "p6_batch_a_r3_geometry_ready"
        if all(row["all_gates_passed"] for row in payload["assets"].values())
        else "p6_batch_a_r3_geometry_failed")
    ...
    (project_root / OUTPUT).write_text(json.dumps(payload, indent=2) + "\n",
                                       encoding="utf-8")
    print(f"[BatchA-R3] status {payload['status']}; wrote {OUTPUT}")
