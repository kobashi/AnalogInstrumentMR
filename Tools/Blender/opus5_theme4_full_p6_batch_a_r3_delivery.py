...
Usage::
    ...
    scripts/run-blender.sh --background --factory-startup \
      --python Tools/Blender/opus5_theme4_full_p6_batch_a_r3_delivery.py -- \
      --project-root "$PWD"
"""
...
import opus5_theme4_full_p6_batch_a_r3 as r3

THEME = "MachinedErgonomics"
TREE = ("ArtSource/Blender/BrushUp/Opus5/MachinedErgonomics/"
        "delivery_p6/batch_a_r3")
OUTPUT = f"{TREE}/theme4_full_p6_batch_a_r3_delivery.json"
...
def main():
    ...
    payload = {
        "phase": "Theme4-P6-BatchA-R3-delivery",
        "note": ("Throttle and PowerSlider with boolean-cut driver "
                 "bores and a split rod/grip arm, through the frozen P4 "
                 "atlas. MeterMedium, MeterLarge, Batch B, batch_a_r2 "
                 "and P1-P5 are untouched."),
        ...
        "budgets": {"triangle_budget_validator_total": r3.VALIDATOR_TOTAL,
                    "triangle_target_total": r3.TARGET_TOTAL},
        "assets": {},
    }

    bleed_hits, bleed_nearest = {}, []
    for asset, builder in r3.BUILDERS_R3.items():
        ...
        measured["triangle_budget_total"] = r3.VALIDATOR_TOTAL
        measured["triangle_target_total"] = r3.TARGET_TOTAL
        measured["gates"]["triangles_total"] = (
            measured["triangles_total"] <= r3.VALIDATOR_TOTAL)
        measured["gates"]["triangles_target"] = (
            measured["triangles_total"] <= r3.TARGET_TOTAL)
        ...
        access = r3.tool_path_audit(body, screws, face, base,
                                    tool_r=r3.BORE_CLEAR_R)
        fbx = tree / f"SM_{asset}_{THEME}_V6_Opus5_P6A_R3.fbx"
        ...
        for label, view in p1.VIEWS.items():
            path = review_dir / f"Delivery_{asset}_{THEME}_P6A_R3_{label}.png"
        ...
        details.update(r3.tool_access_images(asset, screws, face, base,
                                             detail_dir, project_root))

        submeshes = sum(r["submeshes"] for r in rows.values())
        row = dict(measured)
        row.update({
            "p6a_r3_fbx": str(fbx.relative_to(project_root)),
            "p6a_r3_fbx_sha256": m1.digest(fbx),
            "p6a_r3_fbx_bytes": fbx.stat().st_size,
            "objects": rows,
        ...
        print(f"[BatchA-R3-Delivery] {asset}: tris {row['triangles_total']}, "
    ...
    grey = tree / f"ContactSheet_Theme4_{THEME}_P6A_R3_grayscale.png"
    p1.comparison_sheet(rows, grey)
    colour = tree / f"ContactSheet_Theme4_{THEME}_P6A_R3_colour.png"
