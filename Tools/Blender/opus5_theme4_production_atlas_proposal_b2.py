"""Production atlas proposal B2: dedicated normal regions for Theme 4.

Supersedes proposal B1 in the same review cycle. B1 was built against R2,
where the Throttle arm was a single loft, so the tactile pattern ran the whole
length of the rod. R3 splits the arm into `handle_rod`, `handle_ferrule` and
`handle_grip`, and the grip rule below now matches only the grip - the rod and
its ferrule stay plain, which is what the pattern boundary should have been.
...
Usage::
    ...
    scripts/run-blender.sh --background --factory-startup \
      --python Tools/Blender/opus5_theme4_production_atlas_proposal_b2.py -- \
      --project-root "$PWD"
"""
...
TREE = ("ArtSource/Blender/BrushUp/Opus5/MachinedErgonomics/"
        "delivery_p6/batch_a_r3/atlas_proposal")
OUTPUT = f"{TREE}/theme4_production_atlas_proposal_b2.json"
PREFIX = f"T_{THEME}_ProdB2_Atlas"
ATLAS = 2048
...
def materials_for(textures, normal_key, tag):
    atlas = bpy.data.materials.new(f"MAT_{THEME}_ProdB2_Atlas_{tag}")
    wire(atlas, textures, normal_key, False)
    emissive = bpy.data.materials.new(f"MAT_{THEME}_ProdB2_Emissive_{tag}")
...
def main():
    ...
    payload = {
        "phase": "Theme4-ProductionAtlas-Proposal-B2",
    ...
    for asset, builder in r3.BUILDERS_R3.items():
        ...
        print(f"[AtlasProposalB2] {asset}: regions "
    ...
    (project_root / OUTPUT).write_text(json.dumps(payload, indent=2) + "\n",
                                       encoding="utf-8")
    print(f"[AtlasProposalB2] status {payload['status']}; wrote {OUTPUT}")
