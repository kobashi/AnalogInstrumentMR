"""Phase M2n7: the same delivery path as M2n5, pointed at the M2n7 shapes.

Alignment 205.1. Nothing about how a meter is delivered changes here - the
...
The canonical Blends are not the source this time; the M2n7 revisions are. Both
...
Usage::
    ...
    scripts/run-blender.sh --background --factory-startup \
      --python Tools/Blender/opus5_meter_m2n7_delivery.py -- \
      --project-root "$PWD" --mode export --staging /tmp/opus5-m2n7-fbx
"""
...
SUMMARY = "ArtSource/Blender/BrushUp/Opus5/meter_m2n7_handoff.json"
REVISION = "M2n7"
EXPECTED_TRIANGLES = m2n7.EXPECTED_TRIANGLES
...
def point_at_revision(project_root):
    """Redirect the M2n5 delivery at the M2n7 Blends, and rename its outputs."""
    rows = {}
    for key in m2n.SOURCES:
        blend = m2n7.revision_blend(project_root, key)
        if not blend.is_file():
            raise SystemExit(f"[Opus5M2n7Fbx] missing revision blend: {blend}")
    ...
    m2n5.SUMMARY = "ArtSource/Blender/BrushUp/Opus5/meter_m2n7_delivery_report.json"

    def report_path(root, key):
        stem = m2n5.delivered_fbx_name(key).replace("SM_", "").replace(".fbx", "")
        return root / m2l.REPORT_DIR / f"{stem}_m2n7_candidate.json"
...
def needle_after_round_trip(scene):
    ...
    """
    entry = scene.get(m2n7.m2n6.NEEDLE)
    if entry is None:
...
def main():
    ...
    payload = {
        "phase": "M2n7",
        "note": (
            "M2n5 delivery path, M2n7 shapes (alignment 205.1). No new "
    ...
    for key in m2n.SOURCES:
        ...
        payload["models"][key] = {
            ...
            "zone_band_absent": m2n7.m2n6.ZONE_BAND not in scene,
        }
    ...
    print(
        f"[Opus5M2n7Fbx] status {payload['status']}, canonical unchanged "
